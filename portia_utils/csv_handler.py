...
def readcsv(filename: str, separator: str = ',', rstrip: bool = True) -> List[List[str]]:
	...
	lines: List[str] = fh.readfile(filename, rstrip)
...
def readcsv_float_values(
		filename: str,
		separator: str = ',',
		rstrip: bool = True
) -> List[List[float]]:

	list_of_list: List[List[str] or List[float]] = readcsv(
		filename,
		separator=separator,
		rstrip=rstrip
	)
...
def writecsv(
	...
	if directory is not None or directory != '':
		if not fh.is_valid_path(directory):
			os.mkdir(directory)
		complete_file_path = os.path.join(directory, filename)
	else:
		complete_file_path = filename

	fh.writefile(complete_file_path, writecontent)


def writecsv_matrix(datamatrix: m.DataMatrix, filename: str = None):

	list_of_list: List[List[Union[float, str]]] = du.get_classlabeled_list_of_list_from_datamatrix(
		datamatrix
	) if datamatrix.classlabels is not None else du.get_list_of_list_from_datamatrix(
		datamatrix
	)
	attributes: List[str] or None = copy.deepcopy(
		datamatrix.attributes
	) if datamatrix.attributes is not None else None
